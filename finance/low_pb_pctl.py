...
def screen_stocks():
    df = cache.get('low_pb_pctl')
    if df is None:
        print(f"get_gt_3y() at {datetime.datetime.now()}")
        all_df = a_stocks.get_gt_3y()[['代码', '市盈率-动态', '60日涨跌幅', '年初至今涨跌幅']]
        print(f"performance.get() at {datetime.datetime.now()}")
        df = performance.get()
        ...
        del df['每股经营现金流量']
        # del df['最新公告日期']
        print(f"stock_indicator.enrich() at {datetime.datetime.now()}")
        stock_indicator.enrich(df)
        df = df[df['roe|dv|pe|pepctl|pbpctl'].apply(lambda x: x[4] > 0.98)]

        print(f"finance_indicator.enrich() at {datetime.datetime.now()}")
        finance_indicator.enrich(df)
        # df = df[df['存货|应收|天数|净利率'].apply(sum_tuple) > 2]

        print(f"cash_flow_quarter.enrich() at {datetime.datetime.now()}")
        cash_flow_quarter.enrich(df)
        print(f"balance_sheet.enrich() at {datetime.datetime.now()}")
        balance_sheet.enrich(df)
        print(f"income.enrich() at {datetime.datetime.now()}")
        income.enrich(df)
