...
def do_enrich(c):
    try:
        ...
        val = round(math.log(stock_df['roe'].mean() * 100 / 18, 2), 2), \
              1 * round(math.log(stock_df['dv_ttm'][:600].mean() + 0.1, 5), 2), \
              -2 if pe_ttm_de <= 0 else round(math.log(1 / pe_ttm_de / 0.05)*2, 2), \
